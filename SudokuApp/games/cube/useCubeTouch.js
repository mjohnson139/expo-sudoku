  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (event, state) => {
        ...
        if (g.mode === 'undecided') {
          // The first few points of a drag are the ones that say which way it
          // meant to go. Spend as few of them as will still give a stable answer.
          if (Math.hypot(dx, dy) < TUNING.DECIDE_POINTS) return;

          const move = moveForDrag(g.pick, [dx, dy], { size: edge, yaw: y, pitch: p });
          if (!move) return;

          g.mode = 'turn';
          g.move = move;
          g.at = Date.now();
          // A layer under a finger and a scramble playing itself are two things
          // turning the same cube. The finger wins, exactly as a tap on the pad
          // does.
          turns.onPause();
        }
