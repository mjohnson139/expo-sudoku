...
const BUILD_NOTES = {
  ...
  '3.2.0': {
    title: 'Two new games — Number Slide and Color Loop join the hub',
    date: '2026-08-11',
    notes: [
      'New on the hub: Color Loop — drag a row or column and it wraps around the edge',
      'Tapping the card lands you straight on a playable board; the menu button holds everything else',
      'Three goals: every row one solid colour, each row matched to the colour key, or every diagonal one colour',
      'Diagonal counts either direction, and the cover shows you the pattern you are aiming for',
      'Boards from 3×3 to 6×6 — diagonal stops at 4×4, because a diagonal board needs 2n−1 colours',
      'A line follows your finger, wraps around the edge and clicks into its slot',
      'Flick a line and it coasts, slowing into place; catch a spinning line mid-flight and keep dragging it',
      'A grab near the seam between two lines takes both of them at once',
      'The board celebrates first: a wave of tiles rolls across it by diagonal before the win card arrives',
      'Every board has a code, and the code *is* the puzzle — 4-K7P2Q opens the same board on anyone\'s phone',
      'Tap the code chip to copy it; paste a code into the menu to play exactly the board a friend just played',
      'Training: eighteen rungs from a two-move 3×3 up to a full 6×6, three stars a rung, beat one to open the next',
      'Match: Sprint, Classic and Marathon gauntlets where one code deals everybody the same run of boards',
      'A match result copies as a card with your splits and the code inside, so the invitation travels with the brag',
      'Add your name and it rides along on your best times and your result cards',
      'Touch feel lives behind the menu: coast, flick, magnet and twin, saved as you set them',
      'Your Color Loop board is kept as you play it: leave for the hub, close the app, come back — the tiles, the moves and the clock are where you left them',
      'That holds for a training rung and a match too: a half-finished Marathon comes back on the right board with the splits you already ran',
      'The hub card says which one you left — "4×4 in order · 01:24", "Level 9 · 00:41" or "Sprint · 2/3 · 01:07"',
      'The clock stops when you leave and picks up where it stopped, so time spent on the hub is not counted against you',
      'Solve the board and the badge goes away — a finished puzzle is not something to continue',
      'Color Loop\'s tiles are the app\'s own colourblind-safe palette, the same hues Fungiku\'s swatches use',
      'Each hue carries its own glyph, so colour is never the only thing telling two tiles apart',
      'Every colour around the board follows your theme, and each one is checked for contrast on all seven',
      'New on the hub: Number Slide, the classic sliding-tile puzzle',
      'Three board sizes — 3×3, 4×4 and 5×5 — one tap apart, each dealing a fresh board',
      ...
      'Every board has a puzzle code, and the code *is* the puzzle — it carries the size too, so 4-K7P2Q opens a 4×4',
      ...
      'Your board is kept as you play it: leave for the hub, close the app, come back — the tiles, the moves and the clock are where you left them',
      'The hub card shows a Continue badge with the size, the clock and your move count',
      'The clock stops when you leave and picks up where it stopped, so time spent on the hub is not counted against you',
  ...
  '3.1.0': {
    ...
    notes: [
      ...
      'Tap any move in the scramble and the cube turns its way there, forwards or backwards',
      'A speed chip runs the turns at half, normal or double pace',
      ...
      'A live readout names the hold in colours as you turn it: "yellow up · blue left"',
      'The hold is named the way Roux names it — the top colour and the left colour, because the M slice keeps moving the front one',
      ...
      'Undo turns the last move back before removing it',
      'Type or paste a whole algorithm — a CMLL alg, a sequence off a tutorial — and watch it run',
      'What you typed is what you read back: a solve written r U r’ stays r U r’',
      'The transport scrubs a solve exactly as it scrubs a scramble, including tapping a move to turn to it',
      'Solves are kept: background the app, come back, and your solve is where you left it',
      'Come back into the same solve, the same hold and the same mode — nothing is replayed at you',
      'Write several solves against one scramble and switch between them from the notebook button',
      'Name a solve, or duplicate one to try the same first block with a different second block',
      'A solve stays with the scramble it was written for, whether or not that scramble is a favorite',
      'Annotate the move groups: finish your first block, tap the flag, and name it',
      'Roux and CFOP name their own phases — First block, CMLL, Cross, OLL — one tap each',
      'Read the counts straight back: "First block · 8" is what a Roux learner is trying to improve',
      'Tap a group to play just that part of the solve, from where it starts to where it ends',
      'The boundaries show in the moves themselves, so a written solve reads in groups',
      'Name a group again to rename it, or take a marker off and the two groups either side join up',
