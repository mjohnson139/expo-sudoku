...
**Variations keep inactive branches beside the flat active algorithm.** A locked rail pill opens a bottom sheet. Try again stashes the complete continuation from that marker — the selected stage, every later stage, and branches below them — then truncates the active solve to the shared prefix. That is the tree rule learned in the first device pass: later-stage moves must neither survive as the new stage's moves nor disappear from the old complete solve. Choosing the old branch restores the complete continuation; choosing the retry restores that branch. `variations.js` owns fork, switch and deterministic-best selection. All UI writes still use `editOpen`, and move patches use `withMoves`.
