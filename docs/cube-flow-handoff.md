...
**Variations keep inactive branches beside the flat active algorithm.** A locked rail pill opens a bottom sheet. Try again stashes the complete continuation from that marker — the selected stage, every later stage, and branches below them — then truncates the active solve to the shared prefix. That is the tree rule learned in the first device pass: later-stage moves must neither survive as the new stage's moves nor disappear from the old complete solve. The same pass exposed the return-path problem: once the stage reopens, its normal pill tap still has to lock it. A separate visible `+N` button now sits beside that open pill; it opens the saved attempts. Choosing the old branch restores the complete continuation; choosing the retry restores that branch. `variations.js` owns fork, switch and deterministic-best selection. All UI writes still use `editOpen`, and move patches use `withMoves`.

The solve shape now includes branch snapshots `{ id, phaseAt, alg, phases, variations, savedAt }`; snapshot `alg` and `phases` are relative to their fork, while the active solve stays flat. `sanitizeVariations` recursively retains only parseable branches whose `phaseAt` resolves to a surviving marker. It also reads the stage-only shape written by the first Step 7 preview. Old records gain an empty list without changing other authored fields. The rail remains a fixed 28-point horizontal row; expanded detail is a modal and costs the cube **0 points**.
